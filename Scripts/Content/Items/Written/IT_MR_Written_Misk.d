...
func void Use_OldLetter_Godo2Berta()
{
	if (Npc_IsPlayer(self))	{
		...
		Doc_PrintLines(docID,0,OldLetter_Godo2Berta_1);
		Doc_PrintLines(docID,0,"");
		Doc_PrintLines(docID,0,OldLetter_Godo2Berta_2);
		Doc_PrintLines(docID,0,"");
		Doc_PrintLines(docID,0,"");
		Doc_PrintLines(docID,0,OldLetter_Godo2Berta_3);
		Doc_Show(docID);
...
const string BiographyBartosLaran_1 = "Биография Бартоса Ларанского";
const string BiographyBartosLaran_2 = "Бартос Ларанский (1176-1228 годы) известен как автор множества ученых трактатов о магии, истории и мироустройстве. В молодые годы много путешествовал, учился в монастыре Огня в Нордмаре, у друидов Миртаны и шаманов Южных островов.";
const string BiographyBartosLaran_3 = "В зрелом возрасте вернулся в родной Ларан, где подвизался при дворе короля в качестве наставника троих его сыновей.";
const string BiographyBartosLaran_4 = "Широко прославился ученостью, благочестием, пышными пирами и склонностью к слабому полу. Погиб во время нашествия орков, известного как “Битва за Ларан“. Злые языки утверждают, что Бартос был казнен за измену. Однако имеются сведения, что его погребли под собой обломки башни, разрушенной орочьим тараном.";
...
func void Use_BiographyBartosLaran()
{
	if (Npc_IsPlayer(self))	{
		...
		Doc_PrintLines(docID,0,BiographyBartosLaran_1);
		Doc_PrintLine(docID,0,"~~~~~~~~~~~~~~~~");
		Doc_SetFont(docID,0,FONT_Book);
		Doc_PrintLines(docID,0,BiographyBartosLaran_2);
		Doc_SetMargins(docID,-1,30,30,275,30,1);
		Doc_SetFont(docID,1,FONT_Book);
		Doc_PrintLines(docID,1,BiographyBartosLaran_3);
		Doc_PrintLines(docID,1,BiographyBartosLaran_4);
		Doc_Show(docID);
...
const string SmithGuide_1 = "Наставление кузнецу";
const string SmithGuide_2 = "Только что выкованный клинок подобен неопытному юноше, коему еще предстоит стать умелым воином. Дабы проявить все заложенные от природы свойства, человеку требуется воспитание, а мечу - закалка.";
const string SmithGuide_3 = "Клинок накаляй до тех пор, пока цветом и яркостью он не станет подобен предзакатному солнцу. После того плавно и ровно опусти его острием вниз в растопленный жир кротокрыса и держи там, пока сталь не остынет. Неплохо закаляется клинок в обычной родниковой воде, если прежде растворить в ней красную соль.";
const string SmithGuide_4 = "Говорят, что безбожные орки остужают свои клинки в телах еще живых рабов. Но верующему в Инноса такой способ претит.";
...
func void Use_SmithGuide()
{
	if (Npc_IsPlayer(self))	{
		...
		Doc_SetFont(docID,0,FONT_BookHeadline);
		Doc_PrintLine(docID,0,"");
		Doc_PrintLines(docID,0,SmithGuide_1);
		Doc_PrintLine(docID,0,"");
		Doc_SetFont(docID,0,FONT_Book);
		Doc_PrintLines(docID,0,SmithGuide_2);
		Doc_SetMargins(docID,-1,30,20,275,20,1);
		Doc_SetFont(docID,1,FONT_Book);
		Doc_PrintLines(docID,1,SmithGuide_3);
		Doc_PrintLine(docID,1,"");
		Doc_PrintLines(docID,1,SmithGuide_4);
		Doc_Show(docID);
...
const string SunExtract_1 = "ЭКСТРАКТ СОЛНЦА";
const string SunExtract_2 = "Светлый Иннос, коему мы служим, покровительствует не только королевским воинам, кузнецам или алхимикам, но и виноделам. Именно поэтому вино, изготовленное в нашем монастыре - одно из наиболее ценимых во всей Миртане.";
const string SunExtract_3 = "Для лучшего брожения бери только самые спелые ягоды дикого и лесного винограда, собранные в ясный солнечный день и смешанные в равной пропорции. Помолившись Инносу и уложив ягоды в чан, тщательно выдави сок. Потом перелей сок в чистые дубовые бочки и опусти в подвал, высеченный в толще скалы. Лишь там вино сможет хорошо выбродить и дозреть, сконцентрировав в себе накопленный ягодами небесный огонь.";
...
func void Use_SunExtract()
{
	if (Npc_IsPlayer(self))	{
		...
		Doc_SetMargins(docID,0,275,40,30,40,1);
		Doc_SetFont(docID,0,FONT_BookHeadline);
		Doc_PrintLines(docID,0,SunExtract_1);
		Doc_PrintLines(docID,0,"");
		Doc_SetFont(docID,0,FONT_Book);
		Doc_PrintLines(docID,0,SunExtract_2);
		Doc_SetMargins(docID,-1,30,30,275,30,1);
		Doc_SetFont(docID,1,FONT_Book);
		Doc_PrintLines(docID,1,SunExtract_3);
		Doc_Show(docID);
...
const string OnAlchemy_1 = "Об алхимии";
const string OnAlchemy_2 = "Высшая цель алхимической науки заключается в том, чтобы споспешествовать Инносу в установлении Порядка в мире. Преобразуя вещества, делатель упорядочивает их, возводит на высшую ступень, приближает к идеалу. ";
const string OnAlchemy_3 = "И тогда простые, то есть хаотичные, вещества, в коих самих по себе пользы нет ни на грош, обретают чудесные свойства. При этом и душа делателя, приучающего себя к неустанным трудам и строгому распорядку, приближается к тому, что Иннос желает видеть в своих детях.";
...
func void Use_OnAlchemy()
{
	if (Npc_IsPlayer(self))	{
		...
		Doc_SetMargins(docID,0,275,40,30,40,1);
		Doc_SetFont(docID,0,FONT_BookHeadline);
		Doc_PrintLine(docID,0,OnAlchemy_1);
		Doc_PrintLines(docID,0,"");
		Doc_SetFont(docID,0,FONT_Book);
		Doc_PrintLines(docID,0,OnAlchemy_2);
		Doc_SetMargins(docID,-1,30,40,275,40,1);
		Doc_SetFont(docID,1,FONT_Book);
		Doc_PrintLines(docID,1,OnAlchemy_3);
		Doc_Show(docID);
...
const string UnderInnosEye_Print_1 = "   Под Оком Инноса";
const string UnderInnosEye_Print_2 = "  географический трактат";
const string UnderInnosEye_Print_3 = "               Управление:";
const string UnderInnosEye_Print_4 = "         СТРЕЛКИ, Home, End";
...
func void UnderInnosEye_Print(var int docID, var int page)
{
	...
	if (page == 0)	{
		...
		Doc_PrintLine(docID,1,"");
		Doc_PrintLine(docID,1,"       *  *  *");
		Doc_PrintLine(docID,1,UnderInnosEye_Print_1);
		Doc_PrintLine(docID,1,"");
		Doc_SetFont(docID,1,FONT_Manuscript);
		Doc_PrintLine(docID,1,UnderInnosEye_Print_2);
		Doc_SetFont(docID,1,FONT_ManuscriptHeadline);
		Doc_PrintLine(docID,1,"       *  *  *");
		Doc_PrintLine(docID,1,"");
		Doc_PrintLine(docID,1,"");
		...
		Doc_PrintLine(docID,1,UnderInnosEye_Print_3);
		Doc_PrintLine(docID,1,UnderInnosEye_Print_4);
	}
	else	{
...
const string AboutDragons_Print_Cover_1 = "    О   Д Р А К О Н А Х";
const string AboutDragons_Print_Cover_2 = "   Управление: СТРЕЛКИ, Home, End";
...
func void AboutDragons_Print_Cover(var int docID)
{
	...
	Doc_PrintLine(docID,0,AboutDragons_Print_Cover_1);
	Doc_PrintLine(docID,0,"");
	Doc_PrintLine(docID,0,"");
	Doc_PrintLine(docID,0,"");
	Doc_PrintLine(docID,0,"");
	...
	Doc_PrintLine(docID,0,AboutDragons_Print_Cover_2);
};
const string AboutDragons_Print_P01_1 = "Злокозненные крылатые ящеры, что зовутся драконами - суть одно из отвратительнейших Белиаровых порождений. О природе и достоверных случаях встреч с этими гнусными тварями, одним своим существованием оскорбляющими Инноса, и расскажу я, недостойный, в этой книге.";
const string AboutDragons_Print_P01_2 = "Смиренный Флавиус Венгардский, ";
const string AboutDragons_Print_P01_3 = "маг Огня пятого круга.";
const string AboutDragons_Print_P01_4 = "59 год от начала Новой эпохи.";
...
func void AboutDragons_Print_P01(var int docID)
{
	...
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLines(docID,1,AboutDragons_Print_P01_1);
	Doc_PrintLine(docID,1,"");
	Doc_PrintLine(docID,1,"");
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLine(docID,1,AboutDragons_Print_P01_2);
	Doc_PrintLine(docID,1,AboutDragons_Print_P01_3);
	Doc_PrintLine(docID,1,AboutDragons_Print_P01_4);
};
const string AboutDragons_Print_P02_1 = "Доводилось мне слышать от знающих людей, что проклятый Зверь, призванный повелителем Тьмы, о коем говорится в наших священных книгах, и есть дракон. Однако со всей определенностью сие ни в одном достоверном источнике не указано.";
const string AboutDragons_Print_P02_2 = "Природа драконов весьма причудлива и порой самому здравому смыслу противна. ";
const string AboutDragons_Print_P02_3 = "Телом, что лишено волос и покрыто чешуей, они подобны варанам, глорхам, жерхам, которых зовут также снепперами и остерами, и прочим ползучим гадам, с тем лишь различием, что рост драконов огромен и у них имеются крылья.";
const string AboutDragons_Print_P02_4 = "Многие достойные уважения маги и рыцари древних времен, воочию видевшие драконов, уверяют, что рост Белиаровых порождений равен росту десяти-пятнадцати зрелых мужей и даже более, а крылья их втрое превышают паруса самого большого корабля. У нас нет оснований не доверять этим сведениям, дошедшим от нас от очевидцев.";
...
func void AboutDragons_Print_P02(var int docID)
{
	...
	Doc_SetFont(docID,0,FONT_Manuscript);
	Doc_PrintLines(docID,0,AboutDragons_Print_P02_1);
	Doc_PrintLine(docID,0,"");
	Doc_PrintLine(docID,0,"");
	Doc_PrintLines(docID,0,AboutDragons_Print_P02_2);
	
	AboutDragons_SetMerginsR(docID);
	AboutDragons_PrintPageN(docID,4);
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLines(docID,1,AboutDragons_Print_P02_3);
	Doc_PrintLines(docID,1,AboutDragons_Print_P02_4);
};
const string AboutDragons_Print_P03_1 = "Тело драконов, как уже было упомянуто, сплошь покрыто чешуей. В собрании одного лорда, род которого восходит к временам Старой Империи, мне доводилось видеть настоящую драконью чешуйку. Шириной она в две ладони, толщиной с большой палец, а твердостью превосходит лучшую сталь, выплавляемую из магической нордмарской руды. При том она легка, словно сухое дерево.";
const string AboutDragons_Print_P03_2 = "Форму драконья чешуя имеет заостренную, а ежели посмотреть сбоку, то слегка выпуклую. Вероятно, чешуйки на шкуре плотно прилегают одна к другой, образуя непробиваемый панцирь. ";
const string AboutDragons_Print_P03_3 = "Посему немудрено, что герои древности порой носили доспехи, изготовленные из чешуи дракона. Об этом упоминается во многих легендах и преданиях.";
...
func void AboutDragons_Print_P04(var int docID)
{
	...
	Doc_SetFont(docID,0,FONT_Manuscript);
	Doc_PrintLines(docID,0,AboutDragons_Print_P04_1);
	
	AboutDragons_SetMerginsR(docID);
	AboutDragons_PrintPageN(docID,8);
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLines(docID,1,AboutDragons_Print_P04_2);
};
const string AboutDragons_Print_P05_1 = "Помимо могучей брони, драконы вооружены и разнообразными наступательными средствами. Многие древние авторы упоминают о когтях и клыках длиной в локоть, а то и в целое копье. Но все они сходятся на том, что драконы обладают способностью выдыхать смертоносное пламя. ";
const string AboutDragons_Print_P05_2 = "Ничего невероятного в этом нет, если вспомнить, что подобной способностью наделены и вполне обычные существа, известные как огненные вараны или огненные ящеры.";
const string AboutDragons_Print_P05_3 = "Уж их-то доводилось встречать каждому воину, путешественнику или охотнику. Вероятно, эти существа состоят с драконами в дальнем родстве.";
...
func void AboutDragons_Print_P06(var int docID)
{
	...
	Doc_SetFont(docID,0,FONT_Manuscript);
	Doc_PrintLines(docID,0,AboutDragons_Print_P06_1);
	
	AboutDragons_SetMerginsR(docID);
	AboutDragons_PrintPageN(docID,12);
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLines(docID,1,AboutDragons_Print_P06_2);
};
const string AboutDragons_Print_P07_1 = "Также многие упоминают неких ящероподобных существ, что ходят на двух ногах и носят оружие. Будто бы они всюду следуют за драконами, охраняя их самих и кладки яиц, что драконицы откладывают в глубоких пещерах, обогреваемых подземным огнем. Создания эти именуются обыкновенно людоящерами. ";
const string AboutDragons_Print_P07_2 = "Некий маг Воды писал даже, что якобы людоящеры тоже вылупляются из драконьих яиц, и что лишь из одного яйца на сотню или две вылупляется дракон, а из остальных появляются людоящеры, которые впоследствии становятся его слугами и стражами. ";
const string AboutDragons_Print_P07_3 = "Думается, подобные утверждения стоит списать на фантазию, коей маги Воды бывают подвержены более прочих людей. ";
const string AboutDragons_Print_P07_4 = "Мы не можем даже с уверенностью утверждать, делятся ли драконы на самцов и самок, как это свойственно большинство живых созданий. Мой наставник, почтеннейший Пиродориус, был уверен, что каждый из драконов содержит в себе и мужское, и женское начало, а потому способен производить потомство в одиночку. С моей стороны было бы непростительной дерзостью оспаривать мнение учителя.";
...
func void AboutDragons_Print_P08(var int docID)
{
	...
	Doc_SetFont(docID,0,FONT_Manuscript);
	Doc_PrintLines(docID,0,AboutDragons_Print_P08_1);
	
	AboutDragons_SetMerginsR(docID);
	AboutDragons_PrintPageN(docID,16);
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLines(docID,1,AboutDragons_Print_P08_2);
};
const string AboutDragons_Print_P09_1 = "Существуют ли драконы сейчас? Не берусь ни утверждать этого, ни уверять в обратном. В наше время драконы не встречаются. Те “очевидцы“, коих мне доводилось опрашивать, оказывались либо явными лжецами, либо сумасшедшими. А самому свежему свидетельству из тех, что достойны доверия, более семи сотен лет. Вероятно, все драконы вымерли.";
const string AboutDragons_Print_P09_2 = "Может быть, их перебили древние герои, а, может, они переполнили чашу терпения Инноса и он стер их с лика нашего мира, ненадолго забыв о своем договоре с Аданосом.";
const string AboutDragons_Print_P09_3 = "Как бы то ни было, драконы ныне уже не встречаются.";
...
func void AboutDragons_Print_P09(var int docID)
{
	...
	Doc_SetFont(docID,0,FONT_Manuscript);
	Doc_PrintLines(docID,0,AboutDragons_Print_P09_1);
	
	AboutDragons_SetMerginsR(docID);
	AboutDragons_PrintPageN(docID,18);
	Doc_SetFont(docID,1,FONT_Manuscript);
	Doc_PrintLines(docID,1,AboutDragons_Print_P09_2);
	Doc_PrintLine(docID,1,"");
	Doc_PrintLines(docID,1,AboutDragons_Print_P09_3);
};
